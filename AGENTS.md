...
|---|---|---|---|
| `pprof-integrator` | Add `net/http/pprof` per [prompts/pprof_integration.md](prompts/pprof_integration.md) | `/pprof-integrator <repo>` | pprofserver.go + main() changes |
...
| [prompts/prompt_template.txt](prompts/prompt_template.txt) | Action reads `../../../prompts/...`, Skill/MCP read `../../prompts/...`; also bundled by [Dockerfile](Dockerfile) and [skill/build-zip.sh](skill/build-zip.sh) | Prompt structure changes — edit once, all three pick it up |
| [prompts/pprof_integration.md](prompts/pprof_integration.md) | Read by `skill/pprof_integrator/coordinator.py` via `parents[2] / "prompts" / ...` (MCP inherits by importing `coordinator.py`; Action doesn't read it programmatically) | pprof integration guidance changes — edit once, no second copy |
| Python version (currently 3.12) | `action.yml`, `Dockerfile`, `MCP_SETUP.md`, `pyproject.toml`, `action/README.md`, `mcp_tools/README.md`, `skill/README.md`, `skill/INSTALL.md`, distributed ZIP copies (regenerated by `make build-claude-skill`) | Upgrading Python — search for the old version across these files and replace, then run `make test` |
| Project version string | `VERSION` (root — read dynamically by `pyproject.toml` and `mcp_server_http.py`), `action/package.json`, `action/package-lock.json` (x2), `skill/pprof_analyzer/package.json` | Cutting a release — run `make release-tag <version>` instead of editing by hand |
...
**Rebuild the distributed ZIP** (`make build-claude-skill`) after changing any `skill/<name>/` file, a `SKILL.md`, or `prompts/pprof_integration.md`.
...
- **Python environment**: always use the repo's `.venv` (see [Makefile](Makefile)) — never the system interpreter. No `.venv/` yet? Run `make setup-env` (requires Python 3.12).
- Prefer `make` targets (`test`, `test-action`, `lint`, `check`, …). Fall back to `.venv/bin/python`/`.venv/bin/pytest` only when no target fits.
- Python deps: `action/scripts/requirements.txt` (runtime), `requirements-dev.txt` (dev) — installed via `make setup-env`. Tests: `make test` (or `make test-action`/`make test-mcp`/`make test-skill`/`make test-coverage`).
- Node: `pprof-to-md` pinned in `action/package.json`; use `make npm-install-action`.
...
- **Release**: run `make release-tag <version>` (e.g. `make release-tag 0.1.1`) — it rewrites `VERSION` plus the 3 npm manifest files (5 fields total) via [scripts/bump_version.py](scripts/bump_version.py). `pyproject.toml` and `mcp_server_http.py` read `VERSION` dynamically and never need manual edits. It does **not** commit or tag — review the diff, then `git add`, `git commit`, `git tag -a vX.Y.Z -m "..."`, `git push && git push --tags` manually. Pushing the tag triggers `.github/workflows/release.yml`, which builds the skill zip and publishes a GitHub Release.

Before opening a PR: tests pass, the change is minimal and scoped, no new dependencies outside the pinned requirement files, and (for prompt/LLM changes) a real sample profile still produces a valid, applicable patch.
...
- **Dependencies** belong only in `action/scripts/requirements*.txt` / `action/package.json` — no ad hoc version pins elsewhere.
