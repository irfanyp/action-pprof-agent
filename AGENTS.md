...
| Python version (currently 3.12) | `action.yml` (`python-version`), `Dockerfile` (`FROM python:3.12-slim`), `MCP_SETUP.md` (Docker defaults line), `pyproject.toml` (`requires-python`), `action/README.md` (prerequisites), `mcp_tools/README.md` (prerequisites), `skill/README.md` (FAQ), `skill/INSTALL.md` (prerequisites + system requirements heading), `skill/pprof-analyzer-skill/{INSTALL,SKILL_DISTRIBUTION,SIMPLIFIED_DESIGN,IMPLEMENTATION_SUMMARY}.md` (distributed copies — regenerated by `skill/build-zip.sh` / `make build-claude-skill`) | Upgrading Python — search for the old version (e.g. `3.10`, `3.11`) across these files and replace with the new one, then run `make test` to verify |
...
**Rebuild the distributed ZIP** (`make build-claude-skill`, runs `skill/build-zip.sh`, output `skill/pprof-analyzer-skill.zip`, git-ignored) after changing any `skill/<name>/` file, a `SKILL.md`, or `prompts/pprof_integration.md`.
